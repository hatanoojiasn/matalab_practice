...
## よくある失敗への対策
- **辞書紐付けエラー**: `02_make_model.m`で`DataDictionary`をフルパス設定しています。
...
- **slbuildターゲットエラー**: `04_codegen.m`で`SystemTargetFile='grt.tlc'`を明示、さらに`license('test','Simulink_Coder')`で事前判定して未導入環境は明示スキップします。
